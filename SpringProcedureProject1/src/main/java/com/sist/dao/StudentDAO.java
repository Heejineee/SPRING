...
public class StudentDAO {
	...
	/*
	 * 	PreparedStatement : 일반 SQL문장 전송(SELECT, ...)
	...
	public List<StudentVO> studentListData()
	{
		...
		try
		{
			...
			cs.registerOutParameter(1, OracleTypes.CURSOR);	// 타입 맞추는거
			/*
			 * 타입 맞추기
	...
	public void studentInsert(StudentVO vo)
	{
		try
		{
			...
			cs.executeQuery();	// procedure는 항상 executeQuery! (함수 호출 이니깐)
		}catch(Exception ex)
		{
			System.out.println(ex.getMessage());
		}
	...
	public StudentVO studentDetailData(int hakbun)
	{
		...
		try
		{
			...
			cs=conn.prepareCall(sql);
			cs.setInt(1, hakbun);
			cs.registerOutParameter(2, OracleTypes.VARCHAR); 	// 저장공간 만들기(OUT변수니까 공간 만들고 넣기!)
