...
public class BoardDAO {
    ...
    public BoardVO boardUpdateData(int no)
    {
    	...
    	try
    	{
    		...
    		cs.setInt(1, no);
    		// OUT => 저장공간을 만들어 준다
    		cs.registerOutParameter(2, OracleTypes.CURSOR);	// CURSOR는 자바에 없어서 밑에처럼 ResultSet으로 받음
    		cs.executeQuery();
    		// 저장공간에서 값을 가지고 온다
    		ResultSet rs=(ResultSet)cs.getObject(2);	// 위에 registerOutParameter랑 숫자 동일해야 함
    		rs.next();
    		vo.setNo(rs.getInt(1));
    		vo.setName(rs.getString(2));
    		vo.setSubject(rs.getString(3));
    ...
    public boolean boardUpdate(BoardVO vo)
    {
    	...
    	try
    	{
    		...
    		cs.registerOutParameter(6, OracleTypes.VARCHAR); 	// 결과값을 받겠단 소리(문자열로)
    		cs.executeQuery();	// 실행
    		String result=cs.getString(6);
    		bCheck=Boolean.parseBoolean(result);	// true, false로 보내서 boolean으로 형변환!
    ...
    public int boardTotalPage()
    {
    	...
    	try
    	{
    		String sql="SELECT boardTotalPage() FROM project_board";	// FUNCTION은 이렇게 사용!
